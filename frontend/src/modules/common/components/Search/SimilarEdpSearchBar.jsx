...
function SimilarEdpSearchBar() {
  ...
  useEffect(() => {
    ...
    const searchTermFromUrl = queryParams.get("similarEdpQuery");
  ...
  const handleSubmit = (e) => {
    ...
    params.set("similarEdpQuery", newSearchTerm);
    params.delete("page");
    navigate(`${location.pathname}?${params.toString()}`);
    setShowSuggestions(false);
  };
    ...
    <form onSubmit={handleSubmit} className="d-flex w-100" ref={inputRef}>
      <InputGroup className={`${styles.searchBarWrapper} shadow-sm`}>
        <Form.Control
          ...
          autoComplete="off"
          id={styles.searchBar}
          placeholder="Search similar edps"
...
export default SimilarEdpSearchBar;
