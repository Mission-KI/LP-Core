      ...
      <div className="d-flex align-items-center py-1 position-relative h-100">
        <Dropdown show={open} onToggle={() => setOpen(!open)}>
          ...
          <Dropdown.Menu className="border-0 shadow rounded">
            {dataFormats.map((dataFormat, key) => (
              <Dropdown.Item
                key={key}
                as="div"
              ...
              >
                <input
                  ...
                  checked={selected.includes(dataFormat.key)}
                  onChange={() => handleToggle(dataFormat.key)}
                  autoComplete="off"
                />
                <label className="ms-2 mb-0">{dataFormat.key}</label>
              </Dropdown.Item>
            ))}
          </Dropdown.Menu>
        </Dropdown>
