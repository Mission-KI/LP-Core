...
const DataspaceCard = ({ dataSpace, category }) => {
  ...
  const renderTooltip = (props) => (
    <Tooltip id="publisher-tooltip" {...props}>
      {dataSpace.publisher_filters.length > 0 ? (
        <>
          {dataSpace.name}-{t("categories.publishersInTheCategory")}{" "}
          {category.title}:
  ...
  return (
    <div className={`mb-5 px-2`}>
      <Card
        className={`h-100 rounded pointer border-lighter pt-4 px-4 ${styles.card}`}
      >
        <img
          src={dataSpace.image}
        ...
        <a
          href={tileHref}
          className="h5 text-center mb-0 pt-3 txt-primary bold"
        >
          {dataSpace.title}
        </a>
        <p className="pt-3 text-center" style={{ fontSize: "10pt" }}>
          ...
          <Card.Text>
            <div className="d-flex justify-content-between">
              ...
              {!dataSpace.is_publisher && (
                ...
                >
                  <span className="bold pe-1">
                    {dataSpace.amount_of_publishers}
                  </span>
                  {dataSpace.amount_of_publishers === 1
                    ? t("home.dataProvider")
